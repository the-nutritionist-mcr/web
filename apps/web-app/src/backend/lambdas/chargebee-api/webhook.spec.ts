...
describe('the webhook handler', () => {
  ...
  it('rejects the webhook if basic auth fails', async () => {
    ...
    const encodedBasicAuth = Buffer.from(`${basicAuthUser}:${basicAuthPassword}`).toString('base64')
  ...
  it('creates a new customer in cognito when called with a customer created event', async () => {
    /* eslint-disable unicorn/numeric-separators-style */
