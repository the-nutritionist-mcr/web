...
const SwrConfigComponent: FC = props => (
  <SWRConfig value={{ dedupingInterval: 0, provider: () => new Map() }}>{props.children}</SWRConfig>
);
...
beforeEach(() => {
  ...
  root.get('/app-config.json').reply(200, dummyAppConfig).get('/app-config.json')
  root.persist()
...
describe('useResource', () => {
  ...
  it.skip('makes a call to the api with the token attached and returns the response', async () => {
    ...
    }).get('/foo').reply(200, mockResponse)
  ...
  it.skip('create makes a post request that optimistically opdates current dataset', async () => {
    ...
    }).get('/foo').reply(200, mockResponse).post('/foo').reply(200, { 'id': "3" })

    const { result, waitForNextUpdate } = renderHook(() => useResource<{ id: string, foo: string}>('foo'), {
      wrapper: SwrConfigComponent
    });
    ...
    expect(result.current.items).toEqual([...mockItems, { id: "3", foo: 'bip' }])
  })
});
